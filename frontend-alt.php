// Alternative where articels are published as posts
...
$sql = "SELECT journal, volume, issue FROM shelf";
$result = mysqli_query($conn, $sql);
console_log( "Checking database for journals...");
if (mysqli_num_rows($result) > 0) 
{
	console_log( "Results found: " . mysqli_num_rows($result));
    // output 
    while($row = mysqli_fetch_assoc($result)) 
    {
		$journal = str_replace("_", " ", $row["journal"]);
		$pagetitle = $journal . " Vol " . $row["volume"] . " (" . $row["issue"] . ")";
		console_log( "Parsing journal: " . $journal);
        $sql1 = "SELECT title, authors, abstract, pmid, pdflink, weblink FROM " . $row["journal"];
		...
		if (mysqli_num_rows($res1) > 0) 
		{
			// output 
			$content = "";
			console_log( "Articles found: " . mysqli_num_rows($res1));
			while($row1 = mysqli_fetch_assoc($res1)) 
			{
				$t = "<div class='article'><h2>" . $row1["title"]. "</h2>";
				$a = "<h3>" . $row1["authors"]. "</h4>"; 
				$absbutton = "<button class='collapsible'>Abstract</button>";
				$pmidbutton = "<button class='pb' type='button' pmid='" . $row1["pmid"] . "'>Pubmed</button>";
				$pdfbutton = "<button class='rb' type='button' href='" . $row1["pdflink"]. "'>PDF</button>";
				$webbutton = "<button class='rb' type='button' href='". $row1["weblink"]. "'>Web</button>";
				$b =  $absbutton . $pmidbutton . $pdfbutton . $webbutton . "<div class='content'><p>" . $row1["abstract"]. "</p></div></div>";
				$content = $content . "\n" . $t . "\n" . $a . "\n" . $b . "\n";
			}
		} 
		else 
		{
			console_log( "No articles found!");
			$content = "0 results";
		}
		// Create page
		console_log( "Creating page");
		$Page = new PostController;
		// Check if exists
		if ($Page->search('slug', $journal) == TRUE)
		{
			console_log($journal . " already exists, updating");
		...
		else
		{
			...
			console_log($journal . " does not exist, creating");
			$Page->set_title($pagetitle);
			$Page->set_post_slug($journal);
			$Page->set_type("page");
			$Page->set_content($content);
			$Page->set_post_state( "publish" );
			$Page->create();
		}
		//$Page->add_category(array(1,2,8));
		//$Page->set_author_id( 1 );
		//
		//$Page->set_page_template( "login-infusion-page.php" );
	}
}
